...
def llm_judge_query(query: str, results: list[dict], limit: int=5):
    results_strings: list[str] = []
    doc_map = {}
    print(results)
    for doc in results:
        doc_id = doc["id"]
        doc_map[doc_id] = doc
        results_strings.append(f"{doc["title"]} - {doc["document"]}")
...
[2, 0, 3, 2, 0, 1]"""
    ...
    reranked = []
    for i, doc_id in enumerate(doc_map):
        score = int(parsed_ids[i])
        reranked.append({**doc_map[doc_id], "judge_score": score})
